...
| ~~Run 1-bis (2-agent cost A/B via Letta Cloud)~~ | | | | | | | ❌ **superseded by Run 1-anthropic-direct, 2026-05-22.** Letta Cloud runs the same `letta-ai/letta` server code that #3351 lives in, so routing `openrouter/anthropic/*` through Cloud bypasses caching identically to local — no signal to capture. See [t5-run-1-conclusions.md](t5-run-1-conclusions.md) C-6 for the Cloud evaluation. |
| Run 1-anthropic-direct (2-agent cost A/B via native Anthropic) | ~23 min clean (01:51:01 → 02:14:11 UTC) + ~7 min contaminated kickoff (01:38:41 → 01:45:58) | 792,219 | 13,220 | 727,307 | 64,865 | **$1.66** clean / **$2.38** incl. contaminated | ⚠️ **partial — C-6 cache verified, brief not produced.** 60 Anthropic calls total (clean run). Per-service: letta-em (Opus 4.7) 17 calls = $1.25 (132,561 cache_read + 25,770 cache_write + 7,526 out); letta-researcher (Sonnet 4.6) 43 calls = $0.41 (594,746 cache_read + 39,095 cache_write + 5,694 out). Cache hit rate **91.8%** on the live spike workload (vs Run 1's 0% on OpenRouter at $57.87) — the expected 30–50× per-token reduction is real on the cached path. Source: Azure App Insights via OTel auto-instrumentation, query `spike-compose/kql/03-per-agent-cost.kql`. **Halt cause:** Letta's bundled `mcp.client.streamable_http` SSE parser raises `Invalid JSON: EOF while parsing a string` when an SSE frame boundary splits an escaped unicode sequence (`​` inside a Firecrawl-returned Indonesian e-commerce page); the tool result was discarded, Researcher's step hung indefinitely, run cancelled manually at 02:14:11 UTC. Brief.md not produced. See [t5-run-ledger.md](t5-run-ledger.md) Run 1-anthropic-direct row for full failure trace + cancel-API gaps. |
| Run 2 (4-agent main)  | | | | | | | **blocked on Letta MCP streamable-http SSE-parse bug** (see Run 1-anthropic-direct outcome above). The OpenRouter cache bug ([letta-ai/letta#3351](https://github.com/letta-ai/letta/issues/3351)) is now sidestepped by the native-Anthropic switch (cache verified at 91.8% hit, 30–50× cheaper) — Run 2 inherits that route. The new blocker is the MCP framing bug, which breaks any Letta agent doing Firecrawl-backed web research at scale. Unblocks once the parser is fixed upstream or `researcher-web-mcp` is swapped to stdio / chunked-aware client. |
